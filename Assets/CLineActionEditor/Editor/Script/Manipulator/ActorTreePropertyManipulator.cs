...
|
| FILE NAME  : \CLineActionEditor\Editor\Script\Manipulator\ActorTreePropertyManipulator.cs
...
| Ver      Date            By              Details
| -----    -----------    -------------   ----------------------
| 1.0      2021-10-24      SuperCLine           Created
|
+-----------------------------------------------------------------------------*/
...
namespace SuperCLine.ActionEngine.Editor
{
    ...
    internal class ActorTreePropertyManipulator : Manipulator
    {
        ...
        public ActorTreePropertyManipulator(ActorTreeProperty owner)
        {
            this.owner = owner;
        }
        ...
        protected override bool MouseDown(Event evt, ActionWindow window)
        {
            ...
            if (selectable != null)
            {
                ...
                window.SelectProperty(selectable);
                window.AddCaptured(this);

                return true;
            }

            //window.DeselectAllProperty();
        ...
        public override void Overlay(Event evt, ActionWindow window)
        {
            if (dragable != null)
            {
                ...
                rc.x += window.rectInspectorLeft.x;
                rc.y += (window.toobarHeight + window.timeRulerHeight - window.leftScrollPos.y + 50);
